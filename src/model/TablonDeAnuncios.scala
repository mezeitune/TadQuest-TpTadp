...
case class TablonDeAnuncios(misiones: List[Mision]) {
  ...
  def elegirMision(equipo: Equipo, criterio: CriterioMision) = misiones.filter(_.serRealizadaPor(equipo).isSuccess).sortWith((mision1, mision2) => criterio(mision1.serRealizadaPor(equipo).get, mision2.serRealizadaPor(equipo).get)).headOption
